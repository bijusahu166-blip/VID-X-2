...
export function registerAuthRoutes(app: Express): void {
  ...
  app.post("/api/auth/register", async (req: any, res) => {
    try {
      ...
      // Send "new_user" notification to all existing users
      try {
        const allUsers = await db.select({ id: users.id }).from(users).where(
          sql`${users.id} != ${user.id}`
        );
        if (allUsers.length > 0) {
          const notificationPromises = allUsers.map(existingUser =>
            db.insert(notifications).values({
              userId: existingUser.id,
              fromUserId: user.id,
              type: "new_user",
              message: `${firstName} ${lastName} joined the app. Follow to stay updated!`,
            }).catch(err => console.error(`Failed to create notification for ${existingUser.id}:`, err))
          );
          await Promise.all(notificationPromises);
        }
      } catch (notifErr) {
        console.error("Error sending new_user notifications:", notifErr);
 ...
 app.post("/api/auth/login", async (req: any, res) => {
  try {
    ...
    if (!user || !user.password) {
      console.log(`Login attempt: user not found or no password for ${email}`);
