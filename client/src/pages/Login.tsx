...
import { Eye, EyeOff, Loader2, ArrowLeft, KeyRound, CheckCircle2, Mail } from "lucide-react";
...
function ForgotPasswordView({ onBack }: { onBack: () => void }) {
  ...
  const verifyOtp = useMutation({
    ...
    onSuccess: () => { setOtpError(""); setStep("newpw"); },
    onError: (err: Error) => { setOtpError(err.message); setOtp(Array(6).fill("")); inputRefs.current[0]?.focus(); },
  });
  ...
  const handleOtpDigit = (idx: number, val: string) => {
    ...
    const next = [...otp]; next[idx] = d; setOtp(next); setOtpError("");
  ...
  const handleOtpPaste = (e: React.ClipboardEvent) => {
    ...
    if (pasted.length === 6) { setOtp(pasted.split("")); verifyOtp.mutate(pasted); }
  };
  ...
  if (step === "done") {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-sm text-center">
        ...
        <p className="text-zinc-400 text-sm mb-8">Your password has been updated. You can now sign in.</p>
        <button onClick={onBack} className="w-full h-12 rounded-xl font-black text-sm uppercase tracking-widest text-white transition-all"
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="w-full max-w-sm">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => { if (step === "otp") setStep("email"); else if (step === "newpw") setStep("otp"); else onBack(); }}
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500">
            {step === "email" ? "Enter your account email" : step === "otp" ? "Enter OTP sent to your email" : "Choose your new password"}
          </p>
        </div>
      </div>
        ...
        {["Find Account", "Verify OTP", "New Password"].map((label, i) => {
          ...
          return (
            <div key={label} className="flex-1">
              <div className={`h-1 rounded-full mb-1 transition-all ${active ? "bg-red-500" : "bg-zinc-800"}`} />
              <p className={`text-[9px] font-semibold uppercase tracking-wider ${active ? "text-red-400" : "text-zinc-600"}`}>{label}</p>
            </div>
          );
        })}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === "email" && (
          <motion.div key="email" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="mb-4">
              <label className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider mb-1.5 block">Account Email</label>
              <input type="text" inputMode="email" value={email} onChange={(e) => { setEmail(e.target.value); setEmailError(""); }}
                placeholder="you@example.com" onKeyDown={(e) => { if (e.key === "Enter") sendOtp.mutate(); }}
                className={`w-full h-11 px-3.5 rounded-xl bg-zinc-900 border text-white text-sm placeholder:text-zinc-600 focus:outline-none transition-colors ${emailError ? "border-red-500" : "border-zinc-700 focus:border-red-500"}`} />
              {emailError ? (
                <div className="mt-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    {emailError.toLowerCase().includes("no account") && (
                      <p className="text-[11px] text-zinc-500 mt-0.5">Use the exact email you registered with, or{" "}
            ...
            <button onClick={() => sendOtp.mutate()} disabled={!email.trim() || sendOtp.isPending}
        ...
        {step === "otp" && (
          <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            <div className="flex gap-2 justify-center mb-4" onPaste={handleOtpPaste}>
              {otp.map((d, i) => (
                <input key={i} ref={el => { inputRefs.current[i] = el; }} type="text" inputMode="numeric" maxLength={1} value={d}
                  ...
                  className={`w-11 h-14 text-center text-xl font-black rounded-xl border-2 bg-zinc-900 text-white outline-none transition-all ${d ? "border-red-500 text-red-300" : otpError ? "border-red-500/60" : "border-zinc-700 focus:border-red-500"}`}
                  style={d ? { boxShadow: "0 0 10px rgba(239,68,68,0.3)" } : {}} />
              ))}
            </div>
            {otpError && <div className="mb-3 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30"><p className="text-[12px] text-red-400 text-center font-semibold">{otpError}</p></div>}
            <div className="flex items-center justify-center gap-1.5 mb-5">
              <div className={`w-2 h-2 rounded-full ${expired ? "bg-red-500" : secondsLeft <= 60 ? "bg-orange-400 animate-pulse" : "bg-green-400"}`} />
              {expired ? <span className="text-[11px] text-red-400 font-semibold">OTP expired</span> : (
                <span className="text-[11px] text-zinc-500">Expires in <span className={`font-bold ${secondsLeft <= 60 ? "text-orange-400" : "text-white"}`}>{mins}:{String(secs).padStart(2, "0")}</span></span>
              )}
            </div>
            <button onClick={() => { const code = otp.join(""); if (code.length === 6) verifyOtp.mutate(code); }}
            ...
            {expired && (
              <button type="button" onClick={() => { setOtp(Array(6).fill("")); setOtpError(""); sendOtp.mutate(); }}
                className="w-full text-center text-xs text-red-400 hover:text-red-300 transition-colors mt-3 font-semibold">Resend OTP</button>
            )}
          </motion.div>
        )}
        ...
        {step === "newpw" && (
          <motion.div key="newpw" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            <div className="px-3 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800">
            ...
            <div>
              ...
              {newPassword.length > 0 && (
                <div className="flex gap-1 mt-2">
                  {[1,2,3,4].map(i => <div key={i} className={`flex-1 h-1 rounded-full transition-all ${newPassword.length >= i*3 ? i<=1?"bg-red-500":i<=2?"bg-orange-400":i<=3?"bg-yellow-400":"bg-green-400" : "bg-zinc-800"}`} />)}
                </div>
              )}
            </div>
            <div>
              ...
              <div className="relative">
                <input type={showConfirm ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repeat new password"
                  className={`w-full h-11 px-3.5 pr-11 rounded-xl bg-zinc-900 border text-white text-sm placeholder:text-zinc-600 focus:outline-none transition-colors ${confirmPassword && confirmPassword !== newPassword ? "border-red-500/60" : confirmPassword && confirmPassword === newPassword ? "border-green-500/60" : "border-zinc-700 focus:border-red-500"}`} />
              ...
              {confirmPassword && confirmPassword !== newPassword && <p className="text-[11px] text-red-400 mt-1">Passwords do not match</p>}
              {confirmPassword && confirmPassword === newPassword && <p className="text-[11px] text-green-400 mt-1 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Passwords match</p>}
            </div>
            <button onClick={() => resetPassword.mutate()} disabled={!newPassword || !confirmPassword || newPassword !== confirmPassword || resetPassword.isPending}
              className="w-full h-12 rounded-xl font-black text-sm uppercase tracking-widest text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              style={{ background: "linear-gradient(135deg, #ef4444, #f97316)", boxShadow: "0 0 24px rgba(239,68,68,0.4)" }}>
...
export default function Login() {
  ...
  const update = (field: keyof AuthForm, value: string) => setForm((f) => ({ ...f, [field]: value }));

  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-5 relative overflow-hidden">
      <div className="absolute top-[-120px] left-[-80px] w-[340px] h-[340px] rounded-full opacity-20 blur-3xl" style={{ background: "radial-gradient(circle, #ef4444, transparent)" }} />
      <div className="absolute bottom-[-100px] right-[-60px] w-[280px] h-[280px] rounded-full opacity-15 blur-3xl" style={{ background: "radial-gradient(circle, #f97316, transparent)" }} />
          ...
          <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full flex flex-col items-center">
            ...
            <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col items-center mb-10">
              <div className="relative w-20 h-20 mb-4">
                ...
                <img src={logoSrc} alt="VID-X" className="relative w-full h-full rounded-[18px] object-cover shadow-2xl" style={{ boxShadow: "0 0 30px rgba(239,68,68,0.35)" }} />
              </div>
              ...
              <div className="flex rounded-xl overflow-hidden mb-6 border border-zinc-800 bg-zinc-900/60">
                {(["login", "signup"] as Tab[]).map((t) => (
                  <button key={t} onClick={() => { setTab(t); setForm({ email: "", password: "" }); }}
                    ...
                    style={{ background: tab === t ? "linear-gradient(135deg, #ef4444, #f97316)" : "transparent", color: tab === t ? "white" : "#71717a" }}>
