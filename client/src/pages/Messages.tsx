...
// ... (Types, Helpers, Hooks, VoiceNoteBubble, PollBubble wahi hain jo pehle the) ...
// ── Types ─────────────────────────────────────────────────────────────────────
...
// ── Chat List View (FIXED AI TRIGGER) ───────────────────────────────────────────
function ChatList({ 
  onOpenChat, 
  onOpenGroup, 
  pendingOpenChatId,
  ...
  const hiddenBlockedCount = filtered.length - visibleChats.length;

  useEffect(() => {
    if (!hasOpenedPending && pendingOpenChatId && !isLoading && visibleChats.length) {
      const pending = visibleChats.find((chat) => chat.id === pendingOpenChatId);
      if (pending) {
      ...
      <div className="p-4 border-b border-white/10 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={onOpenGroup} className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center">
              <Users className="w-4 h-4" />
            </button>
            <button onClick={() => { setShowNewChat(true); setSearch(""); }} className="w-8 h-8 rounded-full bg-violet-600 flex items-center justify-center">
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {hiddenBlockedCount > 0 ? (
              <div className="space-y-3 px-4 py-4 bg-white/5 rounded-2xl mx-4">
                <p className="text-xs text-zinc-400">
                  {hiddenBlockedCount} blocked conversation{hiddenBlockedCount === 1 ? "" : "s"} hidden. Unblock users to restore chat access.
                </p>
                <button onClick={() => setShowNewChat(true)}
                  className="w-full rounded-2xl bg-violet-600 text-white text-sm font-semibold px-4 py-3 hover:bg-violet-500 transition-colors">
                  Start a new chat
                </button>
              </div>
            ) : (
              <div className="flex flex-col items-center py-16 text-center text-zinc-500">
                <BotMessageSquare className="w-12 h-12 opacity-20 mb-4" />
...
// ── Chat View (Logo wali chat - untouched as requested) ────────────────────────
        ...
        <div className="flex gap-2 px-4 py-2 overflow-x-auto border-t border-white/5">
          {smartReplies.map((s, i) => (
            <button key={i} onClick={() => { setText(s); setSmartReplies([]); }}
              className="shrink-0 text-xs bg-white/10 hover:bg-white/15 border border-white/10 rounded-full px-3 py-1.5 transition-colors">{s}</button>
          ))}
        </div>
      )}

      <div className="flex items-end gap-2 px-3 py-3 bg-black/30 backdrop-blur-sm border-t border-white/10 pb-10">
...
export default function Messages() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col pb-28" style={{ paddingTop: "var(--header-total)" }}>
      <Header />
